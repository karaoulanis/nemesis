...
int SQLiteDatabase::createTable(const char* tableName) {
  ...
  char query[2048];
  sprintf(query, "CREATE TABLE %s (tag INTEGER, id INTEGER, ", tableName);

  char temp[64];
  for (int i = 0; i < myPacket.dblSize; i++) {
    if (i < 10) {
      sprintf(temp, "dbl_0%d  FLOAT, ", i);
    } else {
      sprintf(temp, "dbl_%d   FLOAT, ", i);
    }
    strcat(query, temp);
  }
  for (int i = 0; i < myPacket.intSize; i++) {
    if (i < 10) {
      sprintf(temp, "int_0%d  INTEGER, ", i);
    } else {
      sprintf(temp, "int_%d   INTEGER, ", i);
    }
    strcat(query, temp);
  }
...
void SQLiteDatabase::exportToVtk(const char* tableName) {
  ...
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3] == 0) {
      vtkFile << 0. << " ";
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3]) << " ";
    }
    vtkFile << 0. << " ";
    vtkFile << 0. << " ";
    vtkFile << endl;
  ...
  for (int i = 1; i < nNodes+1; i++) {
    vtkFile << 0. <<" ";
    if (resNodes[i*ncol+2+4] == 0) {
      vtkFile << 0. << " ";
    } else {
      vtkFile << atof(resNodes[i*ncol+2+4]) << " ";
    }
    vtkFile << 0. <<" ";
    vtkFile << endl;
  }
  ...
  for (int i = 1; i < nNodes+1; i++) {
    vtkFile << 0. <<" ";
    vtkFile << 0. <<" ";
    if (resNodes[i*ncol+2+5] == 0) {
      vtkFile << 0. << " ";
    } else {
      vtkFile << atof(resNodes[i*ncol+2+5]) << " ";
    }
    vtkFile << endl;
  }
  vtkFile << endl;
  ...
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3] == 0) {
      ...
      vtkFile << atof(resNodes[i*ncol+2+3]) << endl;
    }
  }
  vtkFile << endl;
  ...
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+4] == 0) {
      ...
      vtkFile << atof(resNodes[i*ncol+2+4]) << endl;
    }
  }
  vtkFile << endl;
  ...
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+5] == 0) {
      ...
      vtkFile << atof(resNodes[i*ncol+2+5]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16+1] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16+1]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16+2] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16+2]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16+3] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16+3]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16+4] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16+4]) << endl;
    }
  }
  vtkFile << endl;
  ...
  vtkFile << "LOOKUP_TABLE DEFAULT" << endl;
  for (int i = 1; i < nNodes+1; i++) {
    if (resNodes[i*ncol+2+3+3*16+5] == 0) {
      vtkFile << 0. << endl;
    } else {
      vtkFile << atof(resNodes[i*ncol+2+3+3*16+5]) << endl;
    }
  }
  vtkFile << endl;
  vtkFile.close();
